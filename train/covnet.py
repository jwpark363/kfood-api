...
# LOGDIR = f"tensorboard/Conv/Conv_20250825_1710"
DATA_DIR = "kfood_processed"  # train/val/test 폴더 루트
SAVE_DIR = "models3"
os.makedirs("models3", exist_ok=True)

SAVE_PATH = "models3/Conv_test_0825_1.pth" # 모델 저장 경로
SAVE_LAST_ALL_PATH = "models3/Conv_last_all_model_0825_1.pth"
BEST_PATH = "models3/best_Conv_test_0825_1.pth" # 최적 모델 저장 경로
CLASS_MAPPING_PATH = "class_mapping.json"   # 클래스 인덱스 매핑
CKPT_PATH = "models3/Conv_checkpoint_0825.pth" # 전체 상테 저장(재개용)

RESUME_WEIGHTS = "models3/Conv_last_model_0825_1.pth"  # 이어서 학습할 가중치
RESUME_CKPT = CKPT_PATH

SAVE_LAST_PATH="models3/Conv_last_model_0825_1.pth"
...
# Mixup / CutMix 관련 , 0825, Conv는 잠깐 꺼야 함(데이터가 안정화 되기 전엔 신호가 희석되서 학습이 안 붙음.)
...
# LOG_DIR = f"tensorboard/Conv/Conv_{datetime.datetime.now().strftime('%Y%m%d_%H%M')}"


LOG_ROOT = "tensorboard"  # 상위 폴더 하나로
RUN_NAME = f"Conv/{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
####### 데이터 전처리(transforms) ########
# Conv 모델의 기본 변환 가져오기
# weights = Conv_Weights.IMAGENET1K_V1
...
# weights = Conv_Weights.IMAGENET1K_V1  # 모델엔 그대로 사용
...
weights = ConvNeXt_Tiny_Weights.IMAGENET1K_V1  # 모델 weights는 그대로 사용
...
# 0824 체크포인트 uv run Conv_0824.py --resume checkpoint 안되서 코드 추가
...
def train_model(model, train_loader, val_loader, use_amp = True):
    ...
    for epoch in range(START_EPOCH, EPOCHS):
        ...
        if writer is not None:
            writer.add_scalar("Loss/train(Conv)", train_loss, epoch)
            if train_acc is not None: writer.add_scalar("Acc/train(Conv)", train_acc, epoch)
            if train_f1 is not None: writer.add_scalar("F1/train(Conv)", train_f1, epoch)
            if val_loss is not None: writer.add_scalar("Loss/val(Conv)", val_loss, epoch)
            writer.add_scalar("Acc/val(Conv)", val_acc, epoch)
            writer.add_scalar("F1/val(Conv)", val_f1, epoch)
        ...
        if (epoch + 1) % SAVE_EVERY == 0:
            torch.save(model.state_dict(), f"models3/model_Conv_epoch{epoch+1}.pth")
...
torch.save(model, "models3/all_0825.pth")
