...
SAVE_PATH = "models/efficientnet/efficientnet_test_0827_1.pth" # 모델 저장 경로
SAVE_LAST_ALL_PATH = "models/efficientnet/efficientnet_last_all_model_0827_1.pth"
BEST_PATH = "models/efficientnet/best_efficientnet_test_0827_1.pth" # 최적 모델 저장 경로
CLASS_MAPPING_PATH = "class_mapping.json"   # 클래스 인덱스 매핑
CKPT_PATH = "models/checkpoint_efficientnet_0827.pth" # 전체 상테 저장(재개용)

RESUME_WEIGHTS = "models/efficientnet_test_0825_1.pth"  # 이어서 학습할 가중치
RESUME_CKPT = "models/checkpoint_efficientnet.pth"

SAVE_LAST_PATH="models/efficientnet/efficientnet_last_model_0827_1.pth"
...
EARLYSTOP_PATIENCE = 13    # EarlyStopping patience
...
LOG_DIR = f"tensorboard/efficientnet/efficientnet_{datetime.datetime.now().strftime('%Y%m%d_%H%M')}"
...
####### 데이터 전처리(transforms) ########
# EfficientNet_V2_S 모델의 기본 변환 가져오기
...
def train_model(model, train_loader, val_loader, use_amp = True):
    ...
    for epoch in range(START_EPOCH, EPOCHS):
        ...
        if writer is not None:
            writer.add_scalar("Loss/train(Efficientnet)", train_loss, epoch)
            if train_acc is not None: writer.add_scalar("Acc/train(efficientnet)", train_acc, epoch)
            if train_f1 is not None: writer.add_scalar("F1/train(efficientent)", train_f1, epoch)
            if val_loss is not None: writer.add_scalar("Loss/val(efficientnet)", val_loss, epoch)
            writer.add_scalar("Acc/val(efficientnet)", val_acc, epoch)
            writer.add_scalar("F1/val(efficientnet)", val_f1, epoch)
        ...
        if (EPOCHS + 1) % SAVE_EVERY == 0:
            torch.save(model.state_dict(), f"models/model_efficient_epoch{epoch+1}.pth")
...
torch.save(model, "models/all_0825.pth")
